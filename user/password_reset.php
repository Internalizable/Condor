...
if(!isset($_COOKIE['pwdReset'])) {
    header("location: ../error?code=301");
    exit;
}
        ...
        <div class="col-md-9 col-lg-6 col-xl-4 px-5 d-flex align-items-center shadow">
            <div class="w-100 py-5">
                <div class="text-center"><img class="img-fluid mb-4" src="../img/back/brand/brand-1.svg" alt="..." style="max-width: 6rem;">
                    <h1 class="h4 text-uppercase mb-3">Reset Password</h1>
                </div>
                <form id="resetForm" class="needs-validation" method="post"
                      oninput='confirmPassword.setCustomValidity(confirmPassword.value != resetPassword.value ? "Passwords do not match." : "")' novalidate>

                    <label class="form-label" for="token"></label>
                    <input class="form-control" id="token" type="text" name="token" value="<?php echo $_COOKIE['pwdReset']?>" disabled readonly hidden>

                    <div class="mb-4">
                        <label class="form-label" for="resetPassword">Password</label>
                        <input class="form-control" id="resetPassword" type="password" name="resetPassword" required>
                    ...
                    <div class="d-grid mb-5">
                        <button id="resetButton" name="reset" class="btn btn-primary text-uppercase">Reset</button>
                    </div>
    ...
    $(document).ready(function() {
        $('#resetButton').click(function(event) {
            ...
            if($('#resetForm')[0].checkValidity()) {
                ...
                $.ajax({
                    url: "../controllers/password_reset.php",
                    type: "POST",
                    data: {
                        'token': $('#token').val(),
