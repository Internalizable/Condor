  ...
  <body>
    <div class="page-holder">
     ...
     <header class="header bg-white">
      ...
      <!--  Modal -->
	 <?php 
	$result = mysqli_query($con, "SELECT * from products where isDeleted=0 order by id asc");
	$media = mysqli_query($con, "select media_path as path from products,products_media where products.isDeleted=0 and products.id=products_media.products_id order by products.id asc ");		
	//$id=$_GET['productid'];
          ...
          <div class="container p-0">
            <div class="row">
              ...
              <div class="col-lg-3 order-2 order-lg-1">
				...
				echo "</strong></div></div>";
				$result1=mysqli_query($con, "SELECT name,id from categories where isDeleted=0 and parent_id=".$row['id']."");
				   ...
				   <?php 
					 // require_once("conn.php");
					  $result = mysqli_query($con, "SELECT * from products where isDeleted=0");		
						...
						$screen = $_GET['screen'];
						$start = ($screen-1) * $rows_per_page;
						$sql = "SELECT * FROM products where isDeleted=0 order by id LIMIT $start, $rows_per_page";
						$r1 = mysqli_query($con, $sql);
						$rows = mysqli_num_rows($r1);
						$media1 = mysqli_query($con, "select media_path as path from  products,products_media where   products.isDeleted=0 and products.id=products_media.products_id order by products.id LIMIT $start, $rows_per_page");
						for ($i = 0; $i < $rows; $i++){
							while ($row = mysqli_fetch_array($r1)) {
								$rmedia= mysqli_fetch_array($media1);
